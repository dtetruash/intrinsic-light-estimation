...
def _get_light_info(config, is_single_olat=False):
    ...
    """
    light_file_path = config.get("datset", "lights_file")
...
def gather_image_metadata(config, split="train"):
    ...
    """
    with open(
        config.get("dataset", "scene_path") + f"/transforms_{split}.json", "r"
    ) as tf:
...
class IntrinsicDataset(Dataset):
    ...
    def __init__(self, config, channels, split="train"):
        ...
        data_path = config.get("dataset", "scene_path") + "/" + self.split
        ...
        for frame_number, frame in pbar:
            ...
            pbar.set_description(config.get("dataset", "scene") + frame["file_path"][1:])
