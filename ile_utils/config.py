...
class Config:
    ...
    def get_config(config_path=None):
        if Config.__conf is None:
            ...
            Config.__conf.set("dataset", "scene_path", "${dataset_path}/${scene}")
            Config.__conf.set("dataset", "data_path", "${scene_path}/${split}")

            if Config.__conf.has_option("dataset", "split"):
                Config.__conf.set(
                    "dataset",
                    "transforms_file",
                    "${scene_path}/transforms_${split}.json",
                )
            else:
                Config.__conf.set(
                    "dataset", "transforms_file", "${scene_path}/transforms.json"
                )

            Config.__conf.set(
                "dataset", "lights_file", "${scene_path}/lights_${scene}.json"
            )
